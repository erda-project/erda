...
func Test_waitOpForLoopNetWorkError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pm1 := monkey.PatchInstanceMethod(reflect.TypeOf(tr), "Update", func(_ *TaskRun) {
				return
			})
			defer pm1.Unpatch()
			pm2 := monkey.PatchInstanceMethod(reflect.TypeOf(tr), "EnsureFetchLatestPipelineStatus", func(_ *TaskRun) {
				return
			})
			defer pm2.Unpatch()
