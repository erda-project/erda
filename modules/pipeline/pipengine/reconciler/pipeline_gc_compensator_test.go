...
func TestReconciler_getNeedGCPipeline(t *testing.T) {
				...
				pipelines: []spec.Pipeline{
					{
						PipelineBase: spec.PipelineBase{
							...
							TimeBegin: func() *time.Time {
								now := now.Add(-190 * time.Second)
								return &now
							}(),
						},
				...
				pipelines: []spec.Pipeline{
					{
						PipelineBase: spec.PipelineBase{
							Status: apistructs.PipelineStatusSuccess,
							TimeBegin: func() *time.Time {
								now := time.Now().Add(-190 * time.Second)
								return &now
							}(),
						},
						PipelineExtra: spec.PipelineExtra{
				...
				pipelines: []spec.Pipeline{
					{
						PipelineBase: spec.PipelineBase{
							Status: apistructs.PipelineStatusSuccess,
							TimeBegin: func() *time.Time {
								addTime := now.Add(-300 * time.Second)
								return &addTime
							}(),
						},
						PipelineExtra: spec.PipelineExtra{
				...
				pipelines: []spec.Pipeline{
					{
						PipelineBase: spec.PipelineBase{
							...
							TimeBegin: func() *time.Time {
								now := now.Add(-time.Duration(spec.DefaultHalfHourSec)*time.Second - 20*time.Second)
								return &now
							}(),
						},
						PipelineExtra: spec.PipelineExtra{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, got1, err := r.getNeedGCPipeline(0, true)
