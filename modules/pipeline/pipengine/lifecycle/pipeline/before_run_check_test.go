...
func Test_matchHookType(t *testing.T) {
	var table = []struct {
		lifecycle []*pipelineyml.HookInfo
		matchLen  int
	}{
		{
			lifecycle: []*pipelineyml.HookInfo{
...
`,
	...
	for _, v := range table {
		...
		guard2 := monkey.Patch(lifecycle.PostLifecycleHookHttpClient, func(source string, req interface{}, resp interface{}) error {
			checkRunResultRequest := req.(CheckRunResultRequest)
			if checkRunResultRequest.Labels != nil {
				assert.Equal(t, checkRunResultRequest.Labels[v.matchOtherLabel], 123)
			}
