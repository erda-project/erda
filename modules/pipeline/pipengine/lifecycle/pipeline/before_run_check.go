...
// 判定HookInfo中是否明确了该类型
func (beforeCheckRun *HttpBeforeCheckRun) matchHookType(lifecycle []*pipelineyml.HookInfo) []*pipelineyml.HookInfo {
	var result []*pipelineyml.HookInfo
...
func (beforeCheckRun HttpBeforeCheckRun) CheckRun() (result *CheckRunResult, err error) {
	...
	labels, err := beforeCheckRun.DBClient.ListLabelsByPipelineID(beforeCheckRun.PipelineID)
	if err != nil {
		return nil, err
	}
	...
	for _, info := range matchInfo {
		...
		// insert the labels on the pipeline into the request
		if info.Labels != nil {
			for key, value := range info.Labels {
				checkRunResultRequest.Labels[key] = value
			}
		}
		checkRunResultRequest.Labels["pipelineID"] = pipeline.ID
		checkRunResultRequest.Labels["pipelineInfo"] = pipeline
		checkRunResultRequest.Labels["pipelineTasks"] = tasks
		checkRunResultRequest.Labels["pipelineLabels"] = labels

		var response CheckRunResultResponse
		err := lifecycle.PostLifecycleHookHttpClient(info.Client, checkRunResultRequest, &response)
		if err != nil {
			return nil, err
		}
