...
func (p *provider) continueBackupQueueUsage(ctx context.Context) {
	...
	for {
		go func() {
			...
			for qID, qMsg := range queueSnapshot.QueueUsageByID {
				if err := p.js.Put(ctx, manager.MakeQueueUsageBackupKey(qID), string(qMsg)); err != nil {
