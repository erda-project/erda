...
func (p *ComponentPodsTable) Render(ctx context.Context, component *apistructs.Component, _ apistructs.ComponentProtocolScenario,
	event apistructs.ComponentEvent, _ *apistructs.GlobalStateData) error {
	if err := p.SetCtxBundle(ctx); err != nil {
		return fmt.Errorf("failed to set podsTable component ctx bundle, %v", err)
	}
	...
	case apistructs.InitializeOperation:
		p.State.PageNo = 1
		p.State.PageSize = 20
	case apistructs.OnChangePageSizeOperation, apistructs.OnChangeSortOperation:
		p.State.PageNo = 1
	}
...
func (p *ComponentPodsTable) SetCtxBundle(ctx context.Context) error {
...
func (p *ComponentPodsTable) GenComponentState(component *apistructs.Component) error {
...
func (p *ComponentPodsTable) DecodeURLQuery() error {
	queryData, ok := p.ctxBdl.InParams["workloadTable__urlQuery"].(string)
	if !ok {
		return nil
	}
...
func (p *ComponentPodsTable) RenderTable() error {
	...
	obj, err := p.ctxBdl.Bdl.GetSteveResource(&req)
	if err != nil {
		return err
	}
	...
	obj, err = p.ctxBdl.Bdl.ListSteveResource(&podReq)
	if err != nil {
		return err
	}
	list := obj.Slice("data")
