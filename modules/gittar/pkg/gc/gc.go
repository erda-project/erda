...
const repositoryRootAddr = "/repository"
...
func doGcCommand(path string) {
	...
	if err != nil {
		logrus.Errorf("gc: command stdoutPipe error: %v", err)
		return
	}
	if err := cmd.Start(); err != nil {
		logrus.Errorf("gc: command start err: %v", err)
		return
	}
	bytes, err := ioutil.ReadAll(stdout)
	if err != nil {
		logrus.Errorf("gc: command readAll stdout error: %v", err)
		return
	}
	if err := cmd.Wait(); err != nil {
		logrus.Errorf("gc: command wait error: %v", err)
		return
	}
	logrus.Infof("gc: end gc path: %v", path)
	logrus.Infof("gc: command stdout:\n\n %s", string(bytes))
}
