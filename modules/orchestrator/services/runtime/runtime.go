...
func (r *Runtime) CreateByReleaseIDPipeline(ctx context.Context, orgid uint64, operator user.ID, releaseReq *apistructs.RuntimeReleaseCreateRequest) (*apistructs.RuntimeDeployDTO, error) {
	releaseResp, err := r.releaseSvc.GetRelease(context.WithValue(ctx, httputil.InternalHeader, "true"), &pb.ReleaseGetRequest{ReleaseID: releaseReq.ReleaseID})
	if err != nil {
		return nil, err
	}
...
func (r *Runtime) RedeployPipeline(ctx context.Context, operator user.ID, orgID uint64, runtimeID uint64) (*apistructs.RuntimeDeployDTO, error) {
	...
	releaseResp, err := r.releaseSvc.GetRelease(context.WithValue(ctx, httputil.InternalHeader, "true"), &pb.ReleaseGetRequest{ReleaseID: deployment.ReleaseId})
	if err != nil {
		return nil, err
	}
	commitid := releaseResp.Data.Labels["gitCommitId"]
...
func (r *Runtime) markOutdated(deployment *dbclient.Deployment) {
	...
	if len(deployment.ReleaseId) > 0 {
		if _, err := r.releaseSvc.UpdateReleaseReference(context.WithValue(context.Background(), httputil.InternalHeader, "true"), &pb.ReleaseReferenceUpdateRequest{
			ReleaseID: deployment.ReleaseId,
			Increase:  false,
		}); err != nil {
