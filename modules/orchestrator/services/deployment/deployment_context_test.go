...
func TestFSMTimeout(t *testing.T) {
	...
	// do invoke
	f.Deployment.CreatedAt = time.Now().Add(-61 * time.Minute)
	end, err := f.timeout()
	if assert.NoError(t, err) {
		assert.True(t, end)
	}

	f.Deployment.CreatedAt = time.Now().Add(-59 * time.Minute)
	end, err = f.timeout()
	if assert.NoError(t, err) {
		assert.False(t, end)
	}
...
func TestFSMGenerateDeployServiceRequest(t *testing.T) {
	...
	monkey.PatchInstanceMethod(reflect.TypeOf(bdl), "GetRuntimeAddonConfig",
		func(_ *bundle.Bundle, req *apistructs.GetRuntimeAddonConfigRequest) (map[string]string, error) {
	...
	monkey.PatchInstanceMethod(reflect.TypeOf(bdl), "FetchDeploymentConfig",
		func(_ *bundle.Bundle, configNamespace string) (map[string]string, error) {
			return map[string]string{
	...
	err := f.generateDeployServiceRequest(&group)
	require.NoError(t, err)

	var actualDice diceyml.Object
	err = json.Unmarshal(group.DiceYml, &actualDice)
	require.NoError(t, err)
	...
	// 2. check request
	group.DiceYml = nil // diceYml is checked
