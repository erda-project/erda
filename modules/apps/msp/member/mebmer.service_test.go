...
import (
	...
	db4 "github.com/erda-project/erda/modules/apps/msp/instance/db"
	db3 "github.com/erda-project/erda/modules/apps/msp/tenant/db"
...
func Test_memberService_ListMemberRoles(t *testing.T) {
	...
	monkey.Patch((*db3.MSPTenantDB).QueryTenant, func(_ *db3.MSPTenantDB, _ string) (*db3.MSPTenant, error) {
		return nil, nil
	})
	monkey.Patch((*db4.InstanceTenantDB).GetInstanceByTenantGroup, func(_ *db4.InstanceTenantDB, _ string) (*db4.InstanceTenant, error) {
		return &db4.InstanceTenant{
			ID:          "1",
			InstanceID:  "",
			Config:      "",
			Options:     `{"projectId":"3"}`,
