...
import (
	...
	common3 "github.com/erda-project/erda/modules/apps/dop/component-protocol/components/requirement-task-overview/common"
...
func init() {
	base.InitProviderWithCreator(common3.ScenarioKey, "simpleChart", func() servicehub.Provider {
		return &SimpleChart{}
	})
}
...
func (s *SimpleChart) Render(ctx context.Context, c *cptype.Component, scenario cptype.Scenario, event cptype.ComponentEvent, gs *cptype.GlobalStateData) error {
	...
	totalIssueUrlQuery, err := common.GenerateUrlQueryParams(conditions)
	if err != nil {
		return err
	}
	start, end := common.MilliFromTime(common3.DateTime(time.Now())), common.MilliFromTime(common3.DateTime(time.Now().AddDate(0, 0, 1)))-1
	conditions.CreatedAtStartEnd = []*int64{&start, &end}
	todayIssueUrlQuery, err := common.GenerateUrlQueryParams(conditions)
	if err != nil {
		return err
	}
	s.Data = Data{
		...
		CompareValue: func() string {
			count := common3.IssueCountIf(s.Issues, func(issue *dao.IssueItem) bool {
				return common3.DateTime(issue.CreatedAt).Equal(common3.DateTime(time.Now()))
			})
			if count > 0 {
	...
	for rd := common3.RangeDate(*itr.StartedAt, *itr.FinishedAt); ; {
		date := rd()
		if date.IsZero() {
			break
		}
		if date.After(common3.DateTime(time.Now())) {
			break
		}
	...
	for k := range dateMap {
		for _, issue := range s.Issues {
			created := common3.DateTime(issue.CreatedAt)
			if !created.After(common3.DateTime(k)) {
				dateMap[k]++
			}
		}
	}
