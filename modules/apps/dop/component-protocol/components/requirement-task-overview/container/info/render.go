...
func init() {
	base.InitProviderWithCreator(common3.ScenarioKey, "info", func() servicehub.Provider {
		return &Info{}
	})
}
...
func (i *Info) Render(ctx context.Context, c *cptype.Component, scenario cptype.Scenario, event cptype.ComponentEvent, gs *cptype.GlobalStateData) error {
	...
	timeMilli := common.TimeMilliInDays{
		Today:               common.MilliFromTime(common3.DateTime(time.Now())),
		Tomorrow:            common.MilliFromTime(common3.DateTime(time.Now().AddDate(0, 0, 1))),
		TheDayAfterTomorrow: common.MilliFromTime(common3.DateTime(time.Now().AddDate(0, 0, 2))),
		SevenDays:           common.MilliFromTime(common3.DateTime(time.Now().AddDate(0, 0, 7))),
		ThirtyDays:          common.MilliFromTime(common3.DateTime(time.Now().AddDate(0, 0, 30))),
	}
...
func buildLink(req ConditionsLinkRequest, expiryType dao.ExpireType) simpleChart.Link {
	...
	urlQuery, err := common.GenerateUrlQueryParams(conditions)
	if err != nil {
