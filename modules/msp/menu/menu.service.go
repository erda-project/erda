...
func (s *menuService) GetMenu(ctx context.Context, req *pb.GetMenuRequest) (*pb.GetMenuResponse, error) {
	...
	if len(clusterName) <= 0 {
		return nil, fmt.Errorf("fail to find cluster by tenantGroup=%s", req.TenantGroup)
	}
	...
	for engine, config := range configs {
		...
		if item == nil {
			return nil, fmt.Errorf("not found menu by key %q", menuKey)
		}
...
func (s *menuService) getEngineConfigs(group, tenantID string) (map[string]map[string]string, error) {
	...
	for _, tenant := range tenants {
		...
		if configs[tenant.Engine] == nil || tenant.ID == tenantID {
			...
			if instance == nil {
				return nil, fmt.Errorf("fail to find instance by id=%s", tenant.InstanceId)
			}
			...
			if len(instance.Config) > 0 {
				...
				if err != nil {
					return nil, fmt.Errorf("fail to unmarshal instance config: %w", err)
				}
			...
			if len(tenant.Config) > 0 {
				...
				if err != nil {
					return nil, fmt.Errorf("fail to unmarshal tenant config: %w", err)
				}
