	...
	if !strings.HasPrefix(metric, "ta_") {
		...
		if len(values) == 0 {
			if terminusKey != "" {
				...
				err := m.db.Raw("SELECT DISTINCT cluster_name FROM sp_monitor WHERE terminus_key=? AND is_delete = 0", terminusKey).Find(&clusters).Error
				if err != nil {
					return nil, err
				}
				if len(clusters) > 0 {
...
func (p *provider) initTkMap() {
	tkMap = make(map[string][]string)
	var maps []TerminusKeyMap
	p.db.Raw("select terminus_key,terminus_key_runtime from sp_monitor where terminus_key_runtime is not null AND is_delete = 0").Find(&maps)
	for _, singleMap := range maps {
		if v, ok := tkMap[singleMap.terminusKey]; ok {
			tkMap[singleMap.terminusKey] = append(v, singleMap.terminusKeyRuntime)
		} else {
			tkMap[singleMap.terminusKey] = []string{singleMap.terminusKeyRuntime}
		}
	}
}
