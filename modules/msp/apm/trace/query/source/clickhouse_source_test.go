...
func TestClickhouseSource_composeFilter(t *testing.T) {
	...
	tests := []struct {
		...
		{"case1", args{req: &pb.GetTracesRequest{TenantID: "test_tenant"}}, "SELECT distinct(series_id) FROM monitor.spans_meta_all WHERE (key='terminus_key' AND value = 'test_tenant') "},
		{"case2", args{req: &pb.GetTracesRequest{TenantID: "test_tenant", ServiceName: "test_service_name"}}, "SELECT distinct(series_id) FROM monitor.spans_meta_all WHERE (key='terminus_key' AND value = 'test_tenant') AND (key='service_name' AND value LIKE concat('%','test_service_name','%')) "},
		{"case3", args{req: &pb.GetTracesRequest{TenantID: "test_tenant", RpcMethod: "hello()"}}, "SELECT distinct(series_id) FROM monitor.spans_meta_all WHERE (key='terminus_key' AND value = 'test_tenant') AND (key='rpc_method' AND value LIKE concat('%','hello()','%')) "},
		{"case4", args{req: &pb.GetTracesRequest{TenantID: "test_tenant", HttpPath: "/hello"}}, "SELECT distinct(series_id) FROM monitor.spans_meta_all WHERE (key='terminus_key' AND value = 'test_tenant') AND (key='http_path' AND value LIKE concat('%','/hello','%')) "},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			chs := &ClickhouseSource{}
