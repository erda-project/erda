...
var (
	// TaskCleanDurationTimestamp job/deployment列表的任务存在时间，默认7天
...
func (e *Endpoints) GenVerifyCode(ctx context.Context, r *http.Request, vars map[string]string) (httpserver.Responser, error) {
	// 鉴权
	identityInfo, err := user.GetIdentityInfo(r)
	if err != nil {
	...
	// 获取验证码
	verifyCode, err := e.org.GenVerifyCode(identityInfo, orgID)
	if err != nil {
		return errorresp.ErrResp(err)
	}
	return httpserver.OkResp(map[string]string{"verifyCode": verifyCode})
}
