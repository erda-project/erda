...
func (f *ComponentAction) Render(ctx context.Context, c *cptype.Component, scenario cptype.Scenario, event cptype.ComponentEvent, gs *cptype.GlobalStateData) error {
	...
	pie := charts.NewPie()
	pie.Colors = []string{"red", "orange", "green"}
	pie.AddSeries("复杂度", common.GroupToPieData(f.State.IssueList, func(issue *dao.IssueItem) string {
		return issue.Complexity.GetZhName()
