...
func init() {
	base.InitProviderWithCreator(common.ScenarioKeyTestDashboard, "at_plan_latest_waterfall_chart_header", func() servicehub.Provider {
		return &Chart{}
	})
}
...
func (f *Chart) Render(ctx context.Context, c *cptype.Component, scenario cptype.Scenario, event cptype.ComponentEvent, gs *cptype.GlobalStateData) error {
	...
	c.Props = common.NewBarProps(values, categories, "自动化测试执行率和通过率走势")
	return nil
}
