...
func (ca *ComponentAction) doListScene(req apistructs.AutotestSceneRequest) error {
	total, list, err := ca.bdl.ListAutoTestScene(req)
	if err != nil {
		return err
	}
	data := []Data{}
...
func (ca *ComponentAction) DeleteScene(ops interface{}) error {
	...
	if err := ca.bdl.DeleteAutoTestScene(ca.State.AutotestSceneRequest); err != nil {
		return err
	}
	return nil
}

func (ca *ComponentAction) UpdateScene(ops interface{}, c *cptype.Component) error {
	mt, err := GetOpsInfo(ops)
	if err != nil {
		return err
	}
...
func (ca *ComponentAction) CopyScene(ops interface{}, c *cptype.Component, inParams InParams) error {
	mt, err := GetOpsInfo(ops)
	if err != nil {
		return err
	}
	...
	_, err = ca.bdl.CopyAutoTestScene(req)
	if err != nil {
		return err
	}
	return nil
...
func getOperation(operationData *ClickRowOperation, event cptype.ComponentEvent) error {
...
func (ca *ComponentAction) ClickScene(event cptype.ComponentEvent) error {
	...
	req.UserID = ca.sdk.Identity.UserID
	scene, err := ca.bdl.GetAutoTestScene(req)
	if err != nil {
		return err
	}
	if scene.RefSetID != 0 {
