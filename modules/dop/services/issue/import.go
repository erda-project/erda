...
func (svc *Issue) storeExcel2DB(request apistructs.IssueImportExcelRequest, issues []apistructs.Issue, instances []apistructs.IssuePropertyRelationCreateRequest, excelIndex []int,
	...
	for index, req := range issues {
		...
		if req.ID > 0 {
			...
			relatings, err := svc.db.GetRelatingIssues(issue.ID)
			if err != nil {
				falseIssue = append(falseIssue, excelIndex[index])
				falseReason = append(falseReason, "failed to get relating issues, err: "+err.Error())
				continue
			}
			...
			relatingMap := map[uint64]bool{}
			for _, re := range relatings {
				relatingMap[re] = true
			}
			...
			for _, issueRelating := range req.GetRelatingIssueIDs() {
				if !relatingMap[issueRelating] {
					_ = svc.db.CreateIssueRelations(&dao.IssueRelation{
						IssueID:      issue.ID,
						RelatedIssue: issueRelating,
					})
				}
			}
			for _, issueRelated := range req.GetRelatedIssueIDs() {
				if !relatedMap[issueRelated] {
					_ = svc.db.CreateIssueRelations(&dao.IssueRelation{
