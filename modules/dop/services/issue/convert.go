...
func (svc *Issue) convertIssueToExcelList(issues []apistructs.Issue, property []apistructs.IssuePropertyIndex, projectID uint64, isDownload bool, stageMap map[issueStage]string, locale string) ([][]string, error) {
	...
	for index, i := range issues {
		...
		relatingIssueIDs, relatedIssueIDs, err := svc.issueRelated.GetIssueRelationsByIssueIDs(uint64(i.ID))
		if err != nil {
			return nil, err
		}
		relatingIssueIDStrs := make([]string, 0)
		for _, id := range relatingIssueIDs {
			relatingIssueIDStrs = append(relatingIssueIDStrs, strconv.FormatUint(id, 10))
		}
		...
		r = append(r, append([]string{
			...
			strings.Join(relatingIssueIDStrs, ","),
		}))
...
func (svc *Issue) decodeFromExcelFile(req apistructs.IssueImportExcelRequest, r io.Reader, properties []apistructs.IssuePropertyIndex) ([]apistructs.Issue,
	...
	for i, row := range rows[1:] {
		...
		// row[16] RelatingIssueIDs
		if err := issue.SetRelatingIssueIDs(row[16]); err != nil {
			falseExcel = append(falseExcel, i+1)
			falseReason = append(falseReason, fmt.Sprintf("failed to convert relating issue ids: %s, err: %v", row[16], err))
			continue
		}
		// row[17] RelatedIssueIDs
		if err := issue.SetRelatedIssueIDs(row[17]); err != nil {
			falseExcel = append(falseExcel, i+1)
