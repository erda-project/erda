...
func (k *Kubernetes) destroyRuntime(ns string) error {
	...
	logrus.Infof("delete the kubernetes namespace %s", ns)
...
func (k *Kubernetes) destroyRuntimeByProjectNamespace(ns string, sg *apistructs.ServiceGroup) error {
	for _, service := range sg.Services {
		...
		if remainCount < 1 {
			logrus.Infof("delete the kubernetes service %s on namespace %s", service.Name, service.Namespace)
			...
			logrus.Infof("delete the kubernetes service %s on namespace %s finished", service.Name, service.Namespace)
		}
	}
	return nil
}
...
func (k *Kubernetes) createStatelessGroup(sg *apistructs.ServiceGroup, layers [][]*apistructs.Service) error {
	...
	for _, layer := range layers {
		...
		for _, service := range layer {
			...
			defer func() {
				logrus.Infof("revert resource when create runtime %s failed", sg.ID)
				var delErr error
				if sg.ProjectNamespace == "" {
