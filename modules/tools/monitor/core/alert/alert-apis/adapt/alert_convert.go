...
func (r *AlertRule) FromModel(lang i18n.LanguageCodes, t i18n.Translator, m *db.AlertRule) *AlertRule {
	...
	window, ok := utils2.GetMapValueInt64(m.Template, "window")
	if !ok {
		return nil
	}
	r.Window = window
	functions, ok := utils2.GetMapValueArr(m.Template, "functions")
	if !ok {
		return nil
	}
	for _, f := range functions {
		...
		field, ok := utils2.GetMapValueString(function, "field")
		if !ok {
			continue
		}
		aggregator, ok := utils2.GetMapValueString(function, "aggregator")
		if !ok {
			continue
		}
		operator, ok := utils2.GetMapValueString(function, "operator")
		if !ok {
			continue
		}
		...
		unit, _ := utils2.GetMapValueString(function, "unit")
...
func ToPBAlertNotify(m *db.AlertNotify, notifyGroupMap map[int64]*pb.NotifyGroup) *pb.AlertNotify {
	...
	notifyType, ok := utils2.GetMapValueString(m.NotifyTarget, "type")
	if !ok {
		return nil
	}
	if notifyType == "notify_group" {
		// notify group ID
		groupID, ok := utils2.GetMapValueInt64(m.NotifyTarget, "group_id")
		if !ok {
			return nil
		}
		// notify group method
		groupType, ok := utils2.GetMapValueString(m.NotifyTarget, "group_type")
		if !ok {
			return nil
		}
		groupLevel, _ := utils2.GetMapValueString(m.NotifyTarget, "level")
		n.Type = notifyType
		n.GroupId = groupID
		...
		dingdingURL, ok := utils2.GetMapValueString(m.NotifyTarget, "dingding_url")
		if !ok {
			return nil
		}
		n.Type = notifyType
...
func ToPBAlertExpressionModel(expression *db.AlertExpression) *pb.AlertExpression {
	...
	ruleIDStr, ok := utils2.GetMapValueString(expression.Attributes, "rule_id")
	if !ok {
		return nil
	}
	...
	e.RuleId = ruleID
	alertIndex, ok := utils2.GetMapValueString(expression.Attributes, "alert_index")
	if !ok {
		return nil
	}
	e.AlertIndex = alertIndex
	e.IsRecover = getRecoverFromAttributes(expression.Attributes)
	window, ok := utils2.GetMapValueInt64(expression.Expression, "window")
	if !ok {
		return nil
	}
	e.Window = window
	level, ok := utils2.GetMapValueString(expression.Attributes, "level")
	if !ok {
		return nil
	}
	e.Level = strings.Title(strings.ToLower(level))

	functions, ok := utils2.GetMapValueArr(expression.Expression, "functions")
	if !ok {
		return nil
	}
...
func ToDBAlertExpressionModel(e *pb.AlertExpression, orgName string, alert *pb.Alert, rule *pb.AlertRule) (*db.AlertExpression, error) {
	...
	filters, _ := utils2.GetMapValueArr(expressionMap, "filters")
	for index, filterValue := range filters {
		filterMap, ok := filterValue.(map[string]interface{})
		if !ok {
			continue
		}
		tag, ok := utils2.GetMapValueString(filterMap, "tag")
		if !ok {
			continue
		}
		operator, ok := utils2.GetMapValueString(filterMap, "operator")
		if !ok {
			continue
		}
		...
		if attr, ok := attributes[tag]; ok {
			val, err := formatOperatorValue(opType, utils2.StringType, attr)
			if err != nil {
				return nil, err
			}
			value = val
	...
	for _, v := range alert.TriggerCondition {
		...
		value := v.Values
		opType := filterOperatorRel[operator]
		val, err := formatOperatorValue(opType, utils2.StringType, value)
		if err != nil {
			return nil, err
		}
		filterMap["tag"] = tag
	...
	functions, _ := utils2.GetMapValueArr(expressionMap, "functions")
	for _, functionValue := range functions {
		function, ok := functionValue.(map[string]interface{})
		if !ok {
			continue
		...
		aggregator, _ := utils2.GetMapValueString(function, "aggregator")
		if !ok {
			continue
		}
		...
		operator, _ := utils2.GetMapValueString(function, "operator")
	...
	if alertDomain == "" {
		if s, ok := utils2.GetMapValueString(attributes, "alert_domain"); ok {
			alertDomain = s
		}
	}
	...
	if routeID, ok := utils2.GetMapValueString(ruleAttributesMap, "display_url_id"); ok {
	...
	if dashboardID, ok := utils2.GetMapValueString(ruleAttributesMap, "alert_dashboard_id"); ok {
		if dashboardPath, ok := utils2.GetMapValueString(alertAttributesMap, "alert_dashboard_path"); ok {
			// get group's value
			var groups []string
			group, _ := utils2.GetMapValueArr(expressionMap, "group")
	...
	if recordPath, ok := utils2.GetMapValueString(alertAttributesMap, "alert_record_path"); ok {
...
func getRecoverFromAttributes(m map[string]interface{}) bool {
	if s, ok := utils2.GetMapValueString(m, "recover"); ok {
