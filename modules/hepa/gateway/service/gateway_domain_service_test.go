...
func TestGatewayDomainServiceImpl_CloudapiDomainExists(t *testing.T) {
	client, err := cloudapi.NewClientWithAccessKey("cn-hangzhou", "LTAI4GGnd4FrdbaidNtPE2ea", "pKAQpWQaUbHpdiNenUAUWFucWvW0s6")
	if err != nil {
		panic(err)
	}
	descDomainReq := cloudapi.CreateDescribeDomainRequest()
	descDomainReq.GroupId = "177e2f4c012a4624afcd3d09676b1e75"
	descDomainReq.DomainName = "zty.test"
	descDomainReq.SecurityToken = uuid.UUID()
	resp, err := client.DescribeDomain(descDomainReq)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", resp)
