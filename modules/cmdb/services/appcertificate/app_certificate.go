...
func (c *AppCertificate) Create(userID string, orgID uint64, createReq *apistructs.CertificateQuoteRequest) error {
	...
	// 获取应用信息
	appInfo, err := c.app.Get(int64(createReq.AppID))
	if err != nil {
	...
	newApproval, err := c.approve.Create(userID, &approvalCreateReq)
	if err != nil {
		return err
	}
...
func (c *AppCertificate) Delete(appID, certificateID int64) error {
	...
	if err := c.db.CancelQuoteCertificate(certificate.ID); err != nil {
