...
func (a *Application) Delete(applicationID int64) (*model.Application, error) {
	...
	// 删除ConfigNamespaces
	a.deleteExtraInfo(application.Extra)
	...
	if err = a.db.DeleteIssueAppRelationsByApp(applicationID); err != nil {
		logrus.Warnf("failed to delete mr relations, %v", err)
	}

	if err = a.db.DeleteBranchRuleByScope(apistructs.AppScope, applicationID); err != nil {
...
// PrepareAbilityApp 能力相关
// TODO  addon-platform支持通过scheduler直接部署能力后，删除
func (a *Application) PrepareAbilityApp(req types.AbilityAppReq) (int64, error) {
	...
	// 项目不存在，创建项目
	project, err = a.db.GetProjectByOrgAndName(req.OrgID, "ability")
	if err != nil {
		if !gorm.IsRecordNotFoundError(err) {
			return 0, err
		}
		clusterConfig := make(map[string]string, 4)
		clusterConfig["DEV"] = req.ClusterName
		clusterConfig["TEST"] = req.ClusterName
		clusterConfig["STAGING"] = req.ClusterName
		clusterConfig["PROD"] = req.ClusterName

		clusterConfigStr, _ := json.Marshal(clusterConfig)
		...
		project = &model.Project{
			...
			OrgID:         req.OrgID,
			UserID:        req.UserID,
			ClusterConfig: string(clusterConfigStr),
		}
		if err = a.db.CreateProject(project); err != nil {
			return 0, nil
		}
		// 添加项目管理员
		member := &model.Member{
	...
	// 应用不存在，创建应用
	applicationName := strutil.Concat("ability-", req.ApplicationName)
	app, err = a.db.GetApplicationByOrgAndName(req.OrgID, applicationName)
	if err != nil {
		if !gorm.IsRecordNotFoundError(err) {
			return 0, err
		}
		app = &model.Application{
			...
			GitRepo:       strutil.Concat("ABILITY-", uuid.UUID()),
			GitRepoAbbrev: strutil.Concat("ABILITY-", uuid.UUID()),
			OrgID:         req.OrgID,
			ProjectID:     project.ID,
			UserID:        req.UserID,
		}
		...
		// 更新extra信息
		app.Extra = a.generateExtraInfo(app.ID, app.ProjectID)
		if err = a.db.UpdateApplication(app); err != nil {
			logrus.Errorf("failed to update application extra, (%v)", err)
		}
...
func (a *Application) generateExtraInfo(applicationID, projectID int64) string {
	...
	for _, v := range extra {
		...
		// 创建namespace
		_, err := a.ns.Create(&namespaceCreateReq)
		if err != nil {
			logrus.Errorf(err.Error())
		}
	}
	...
	if err := a.ns.CreateRelation(&relationCreateReq); err != nil {
		logrus.Errorf(err.Error())
	}
...
func (a *Application) deleteExtraInfo(extra string) {
	...
	for _, v := range extraMap {
		...
		if strings.Contains(v, string(types.DefaultWorkspace)) {
			if err := a.bdl.DeleteNamespaceRelation(v); err != nil {
				logrus.Warnf(err.Error())
			}
		}
		// 删除 namespace
		if err := a.bdl.DeleteNamespace(v); err != nil {
			logrus.Warnf(err.Error())
		}
	}
}
