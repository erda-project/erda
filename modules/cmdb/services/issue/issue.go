...
func (svc *Issue) Create(req *apistructs.IssueCreateRequest) (*dao.Issue, error) {
	...
	go func() {
		...
		project, _ := svc.db.GetProjectByID(int64(create.ProjectID))
...
func (svc *Issue) Paging(req apistructs.IssuePagingRequest) ([]apistructs.Issue, uint64, error) {
	...
	if len(requirementIDs) > 0 {
		...
		for _, r := range requirements {
			requirementTitleMap[r.ID] = r.Title
		}
...
func (svc *Issue) CreateStream(updateReq apistructs.IssueUpdateRequest, streamFields map[string][]interface{}) error {
	...
	for field, v := range streamFields {
		...
		case "stage":
			...
			project, err := svc.db.GetProjectByID(int64(issue.ProjectID))
			if err != nil {
				return err
			}
			stage, err := svc.db.GetIssuesStage(project.OrgID, issue.Type)
			for _, s := range stage {
		...
		go func() {
			...
			projectModel, _ := svc.db.GetProjectByID(int64(issue.ProjectID))
			content, _ := issuestream.GetDefaultContent(streamReq.StreamType, streamReq.StreamParams)
			ev := &apistructs.EventCreateRequest{
				EventHeader: apistructs.EventHeader{
					Event:         bundle.IssueEvent,
					Action:        bundle.UpdateAction,
					OrgID:         strconv.FormatInt(projectModel.OrgID, 10),
...
func (svc *Issue) GetIssueBugStatusPercentage(req apistructs.IssuesStageRequest) ([]apistructs.IssueBugStatusPercentageResponse, error) {
	...
	for _, state := range issueState {
		stateName[state.ID] = state.Name
	}

	ans := []apistructs.IssueBugStatusPercentageResponse{}
	for k, v := range bugs {
		if k == "" {
			continue
		}
...
func (svc *Issue) GetIssueBugSeverityPercentage(req apistructs.IssuesStageRequest) ([]apistructs.IssueBugSeverityPercentageResponse, error) {
	...
	ans := []apistructs.IssueBugSeverityPercentageResponse{}
	for k, v := range bugs {
		if k == "" {
			continue
		}
