...
func (svc *Issue) BatchConvert(models []dao.Issue, issueTypes []apistructs.IssueType, identityInfo apistructs.IdentityInfo) ([]apistructs.Issue, error) {
	...
	// 查询项目下所有 labels
	_, labels, err := svc.db.ListLabel(&apistructs.ProjectLabelListRequest{
	...
	labelMap := make(map[uint64]dao.Label, len(labels))
	...
	for _, model := range models {
		results = append(results, *issueMap[model.ID])
	}
	return results, nil
}
...
func (svc *Issue) ConvertWithoutButton(model dao.Issue,
	...
	needQueryLabels bool, projectLabels map[uint64]dao.Label,
) *apistructs.Issue {
	// 标签
	if needQueryLabelRef {
		lrs, _ := svc.db.GetLabelRelationsByRef(apistructs.LabelTypeIssue, uint64(model.ID))
	...
	if needQueryLabels {
		labels, _ := svc.db.GetLabels(labelIDs)
	...
	return &apistructs.Issue{
		...
		Type:           model.Type,
		Title:          model.Title,
		Content:        filehelper.FilterFilePath(model.Content),
		State:          model.State,
