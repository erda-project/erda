...
func (e *Endpoints) CreateApplication(ctx context.Context, r *http.Request, vars map[string]string) (
	...
	// 添加应用至DB
	app, err := e.app.CreateWithEvent(userID.String(), &applicationCreateReq)
	if err != nil {
...
// TODO addon-platform支持通过scheduler直接部署能力后，删除此API
func (e *Endpoints) PrepareAbilityApp(ctx context.Context, r *http.Request, vars map[string]string) (
	httpserver.Responser, error) {
	var req types.AbilityAppReq
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		return apierrors.ErrCreateApplication.InvalidParameter("can't decode body").ToResp(), nil
	}
	logrus.Infof("request body: %+v", req)

	appID, err := e.app.PrepareAbilityApp(req)
	if err != nil {
		return apierrors.ErrCreateApplication.InternalError(err).ToResp(), nil
	}

	return httpserver.OkResp(appID)
}
