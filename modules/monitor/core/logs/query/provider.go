...
func (d *define) Service() []string { return []string{"logs-query"} }
func (d *define) Dependencies() []string {
	return []string{"http-server", "cassandra", "metrics-query", "cassandra-manager"}
}
...
func (p *provider) Init(ctx servicehub.Context) error {
	...
	if q, ok := ctx.Service("metrics-query").(metricq.Queryer); !ok {
		return fmt.Errorf("failed to init metricq")
	} else {
		p.q = q
	}
