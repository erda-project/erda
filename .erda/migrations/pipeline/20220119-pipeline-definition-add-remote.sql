ALTER TABLE `pipeline_definition` ADD COLUMN  `location` varchar(100) NOT NULL DEFAULT '' COMMENT '地址(org/project)';
ALTER TABLE `pipeline_source` ADD COLUMN  `location` varchar(100) NOT NULL DEFAULT '' COMMENT '地址(org/project)';
...
UPDATE pipeline_source set location = SUBSTRING_INDEX(remote,'/',2);
