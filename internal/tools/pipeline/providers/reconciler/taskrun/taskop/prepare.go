...
func (pre *prepare) makeTaskRun() (needRetry bool, err error) {
	...
	for k, v := range action.Params {
		task.Extra.PrivateEnvs[env.GenPipelineEnvKey(actionagent.EnvActionParamPrefix, k)] = fmt.Sprintf("%v", v)
	}
	// secrets -> envs
	for k, v := range p.Snapshot.Secrets {
		task.Extra.PrivateEnvs[env.GenPipelineEnvKey("", k)] = v
		task.Extra.PrivateEnvs[env.GenPipelineEnvKey(env.EnvPipelineSecretPrefix, k)] = v
	}
	// platform secrets -> envs
	for k, v := range p.Snapshot.PlatformSecrets {
		newK := env.GenPipelineEnvKey("", k)
