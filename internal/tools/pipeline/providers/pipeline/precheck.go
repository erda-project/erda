...
func (s *pipelineService) PreCheck(p *spec.Pipeline, stages []spec.PipelineStage, userID string, autoRun bool) error {
	...
	_, actionSpecs, err := s.actionMgr.SearchActions(extSearchReq)
	if err != nil {
		return apierrors.ErrPreCheckPipeline.InternalError(err)
	}
