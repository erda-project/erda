...
stages:
  ...
  - stage:
	...
	pm := monkey.PatchInstanceMethod(reflect.TypeOf(dbClient), "UpdatePipelineShowMessage", func(_ *dbclient.Client, pipelineID uint64, showMessage basepb.ShowMessage, ops ...dbclient.SessionOption) error {
		return nil
	})
	defer pm.Unpatch()
	svc := &pipelineService{actionMgr: &mockActionAgent{}, secret: &mockSecret{}, dbClient: dbClient, resource: &resourcemgr.MockResource{}}
