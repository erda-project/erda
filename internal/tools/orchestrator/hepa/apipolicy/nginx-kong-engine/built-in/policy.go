...
func (policy Policy) UnmarshalConfig([]byte) (apipolicy.PolicyDto, error, string) {
	return nil, nil, ""
}

// forValidate 用于识别解析的目的，如果解析是用来做 nginx 配置冲突相关的校验，则关于数据表、调用 kong 接口的操作都不会执行
func (policy Policy) ParseConfig(_ apipolicy.PolicyDto, ctx map[string]interface{}, forValidate bool) (apipolicy.PolicyConfig, error) {
...
func init() {
	err := apipolicy.RegisterPolicyEngine(apipolicy.ProviderNKE, apipolicy.Policy_Engine_Built_in, &Policy{BasePolicy: &apipolicy.BasePolicy{}})
	if err != nil {
		panic(err)
	}
}
