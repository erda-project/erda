...
func TestPolicy_setIngressAnnotations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "Test_01",
			fields: fields{BasePolicy: &apipolicy.BasePolicy{Name: "cors"}},
			args: args{
				gatewayProvider: "MSE",
				policyDto: &PolicyDto{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := policy.setIngressAnnotations(tt.args.locationSnippet); !reflect.DeepEqual(got, tt.want) {
...
func TestPolicy_CreateDefaultConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				BasePolicy: &apipolicy.BasePolicy{
					Name: apipolicy.Policy_Engine_CORS,
				},
			},
			args: args{
				gatewayProvider: apipolicy.ProviderNKE,
				ctx:             nil,
			},
			want: &PolicyDto{
				Methods:     "GET, PUT, POST, DELETE, PATCH, OPTIONS",
