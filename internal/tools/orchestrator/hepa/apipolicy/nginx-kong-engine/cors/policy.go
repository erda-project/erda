...
`, coreSnippet, policyDto.MaxAge, coreSnippet)
	...
	res.IngressAnnotation = policy.setIngressAnnotations(locationSnippet)
	emptyStr := ""
...
func (policy Policy) setIngressAnnotations(locationSnippet string) *apipolicy.IngressAnnotation {
	return &apipolicy.IngressAnnotation{
		Annotation: map[string]*string{
			string(annotationscommon.AnnotationEnableCORS):           nil,
			string(annotationscommon.AnnotationCORSAllowMethods):     nil,
			string(annotationscommon.AnnotationCORSAllowHeaders):     nil,
...
func init() {
	err := apipolicy.RegisterPolicyEngine(apipolicy.ProviderNKE, apipolicy.Policy_Engine_CORS, &Policy{BasePolicy: &apipolicy.BasePolicy{}})
	if err != nil {
		panic(err)
	}
}
