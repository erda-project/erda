...
func (p *Parser) ParseOrderByOnExpr(s influxql.SortFields, expr *goqu.SelectDataset, selectColumns *_columns, groupColumns *_columns) (*goqu.SelectDataset, error) {
	...
	var firstOrderExpress []exp.OrderedExpression
	...
	if len(s) > 0 {
		for _, field := range s {
			...
			if column == timeBucketColumn {
				firstOrderExpress = append(firstOrderExpress, goqu.C(timeBucketColumn).Asc())
				continue
			}
			if column == "*" {
				continue
	...
	for _, express := range firstOrderExpress {
		expr = expr.OrderAppend(express)
	}

	for _, column := range copiedColumns {
		if column.isWildcard || column.asName == timeBucketColumn {
			continue
		}
	...
	for _, dim := range dimensions {
		...
		case *influxql.Call:
			if expr.Name == "time" {
				...
				bucketTimeKey := selectColumns.addTimeBucketColumn(p.ctx.TimeKey(), intervalSeconds)
				groupColumn.addTimeBucketColumn(p.ctx.TimeKey(), intervalSeconds)

				exprSelect = exprSelect.SelectAppend(goqu.L(fmt.Sprintf("addSeconds(toDateTime('1970-01-01 00:00:00'),%s * %v)", bucketTimeKey.expr, intervalSeconds)).As(timeBucketColumn))
