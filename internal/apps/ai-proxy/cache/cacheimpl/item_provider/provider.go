...
func NewProviderCacheItem(dao dao.DAO, config *config.Config) cachetypes.CacheItem {
	item := &providerCacheItem{}
	item.BaseCacheItem = cachetypes.NewBaseCacheItem(cachetypes.ItemTypeProvider, dao, config, item)
	return item
}
...
func (c *providerCacheItem) QueryFromDB(ctx context.Context) (uint64, any, error) {
	...
	if err != nil {
		return 0, nil, fmt.Errorf("failed to query providers: %w", err)
	}
