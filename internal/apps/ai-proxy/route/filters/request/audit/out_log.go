...
func (f *Audit) outLog(out *http.Request) error {
	...
	if err != nil {
		return fmt.Errorf("failed to dump request out, err: %v", err)
	}

	logger := ctxhelper.MustGetLogger(out.Context())
	if shouldDumpBody {
		logger.Infof("audit proxy request out:\n%s", dumpBytesOut)
	} else {
		logger.Infof("audit proxy request out (body omitted due to binary content-type: %s):\n%s",
			contentType, dumpBytesOut)
	}
	return nil
}
