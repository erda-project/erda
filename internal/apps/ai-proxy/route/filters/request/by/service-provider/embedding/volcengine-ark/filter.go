...
func init() {
	filter_define.RegisterFilterCreator("volcengine-ark-multimodal-embedding-converter", Creator)
}

type VolcengineMultimodalEmbeddingConverter struct{}

var Creator filter_define.RequestRewriterCreator = func(_ string, _ json.RawMessage) filter_define.ProxyRequestRewriter {
	return &VolcengineMultimodalEmbeddingConverter{}
}

func (f *VolcengineMultimodalEmbeddingConverter) Enable(pr *httputil.ProxyRequest) bool {
	return Enabled(pr.In.Context())
}

func Enabled(ctx context.Context) bool {
	pathMatcher := ctxhelper.MustGetPathMatcher(ctx)
	if pathMatcher.Pattern != vars.RequestPathPrefixV1MultimodalEmbedding {
		return false
	}
