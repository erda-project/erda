...
// getTargetProvider extracts target provider from model metadata
func (f *ThinkingHandler) getTargetProvider(modelMeta metadata.Metadata) string {
	// try to get provider name from metadata
	providerName := ""
	if public := modelMeta.Public; public != nil {
		if provider, ok := public["provider"]; ok {
	...
	case "doubao":
		return types.ProviderDoubao
	default:
		return strings.ToLower(providerName)
	}
}

// getTargetAPI extracts target API type from model metadata
func (f *ThinkingHandler) getTargetAPI(modelMeta metadata.Metadata) string {
	// try to get API segment path from metadata
	apiPath := ""
	if public := modelMeta.Public; public != nil {
		if apiSegment, ok := public["api_segment"]; ok {
			if segmentMap, ok := apiSegment.(map[string]any); ok {
				if path, ok := segmentMap["path"].(string); ok {
					apiPath = path
				}
			}
		}
	...
	if apiPath == "" {
		return types.APIChat // default to chat
	}

	// extract API type from API segment path
	apiPathLower := strings.ToLower(apiPath)

	if strings.Contains(apiPathLower, "responses") {
		return types.APIResponses
	}
	if strings.Contains(apiPathLower, "realtime") {
		return types.APIRealtime
	}

	// default to chat for most APIs
	return types.APIChat
}
