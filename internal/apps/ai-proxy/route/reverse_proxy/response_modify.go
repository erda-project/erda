...
func asyncHandleRespBody(upstream io.ReadCloser, splitter filter_define.RespBodyChunkSplitter, pw *io.PipeWriter, resp *http.Response, filters []filter_define.FilterWithName[filter_define.ProxyResponseModifier], peekedChunk []byte, peekedChunkIsEOF bool) {
	...
	defer func() {
		...
		wholeReceivedBody = optimizeBodyForAudit(wholeReceivedBody, bodyHeadPersistBytes, bodyTailPersistBytes)
		wholeHandledBody = optimizeBodyForAudit(wholeHandledBody, bodyHeadPersistBytes, bodyTailPersistBytes)
...
// optimizeBodyForAudit replaces truncateBodyForAudit with an SSE-aware strategy.
