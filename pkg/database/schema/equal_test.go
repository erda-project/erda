...
func TestFieldTypeEqual(t *testing.T) {
	...
	//}
	if eq := schema.FieldTypeEqual(i.Tp, j.Tp); eq.Equal() {
		t.Fatal("error assert equal between col i and col j")
	}
	if eq := schema.FieldTypeEqual(i.Tp, k.Tp); eq.Equal() {
		t.Fatal("error assert equal between col i and col k")
	}
	if eq := schema.FieldTypeEqual(b.Tp, l.Tp); eq.Equal() {
